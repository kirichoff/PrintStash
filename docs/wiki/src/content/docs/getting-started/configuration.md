...
selects the image tag the default `docker-compose.yml` pulls (e.g. `0.6.4`). Pin
it for reproducible deploys and bump it to upgrade; if unset it defaults to
`latest`.
...
| `VAULT_MESH_MAX_LOAD_MB`            | `200`                                | Hard ceiling on the on-disk size of a mesh file handed to the loader. The triangle estimate above is format-specific and can come up empty (e.g. a large 3MF whose mesh parts it can't size up); this byte cap is the format-blind backstop so such a file is never decompressed into memory and can't OOM-kill a scan. The file is still indexed, and 3MF keeps its embedded preview. Set to `0` to disable. |
| `VAULT_MESH_MEMORY_BUDGET_FRACTION` | `0.5`                                | Fraction of detected available RAM a single mesh load+render may peak to. The effective triangle cap is derived from this (per format, using measured per-triangle peak cost), divided by `VAULT_MAX_RENDER_JOBS`, and combined with `VAULT_MESH_MAX_RENDER_TRIANGLES` via a `min()`, so a small (e.g. 4 GB) container automatically skips meshes a 32 GB host would render — no per-host tuning needed to avoid an OOM-killed scan. Container-aware: honours the cgroup memory limit, not just host RAM. `0.5` keeps headroom for the rest of the app; `0.30`–`0.35` is safer for production / self-hosted boxes running other workloads. Set to `0` to disable RAM-aware capping. |
| `VAULT_MAX_RENDER_JOBS`             | `1`                                  | Maximum mesh load+render jobs allowed to run at once. Bulk/folder uploads run in a background-task threadpool, so without this a single upload could fire many concurrent renders that each peak hundreds of MB and collectively OOM the box. A semaphore caps simultaneous renders to this value, and the RAM-aware triangle cap divides its per-job budget by it. `1` (serialised) is safest; raise it on hosts with RAM headroom. |
| `VAULT_MESH_RENDER_FACE_CHUNK_SIZE` | `200000`                             | Number of faces the software rasteriser processes per chunk. Per-face geometry/shading arrays are built and freed one chunk at a time, so peak render memory is O(chunk size) instead of O(total faces) — a million-triangle mesh no longer allocates its full per-face arrays at once. Lower it to shrink peak RSS further on tiny containers; raise it for marginally less Python-loop overhead. |
| `VAULT_USE_EMBEDDED_3MF_PREVIEW_FOR_LARGE_FILES` | `true`                  | For a 3MF whose estimate exceeds the adaptive cap, use its embedded slicer preview directly and never hand the archive to the loader (whose XML parse is the dominant memory cost). Set to `false` to skip the embedded fallback for over-cap 3MFs (they then get no thumbnail). |
